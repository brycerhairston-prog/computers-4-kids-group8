...
const ShotTracker = () => {
  ...
  const shotCountDisplay = (() => {
    ...
    if (gameMode === "individual") {
      ...
      const playerCount = getPlayerShotCount(activePlayerId);
      const playerLimit = getPlayerShotLimit(activePlayerId);
      return t("shotTracker.teamPlayerCount", {
        team: team.name,
        teamCount,
        teamLimit: TEAM_SHOT_LIMIT,
  ...
  const handleCourtClick = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    // Pass percentages to logic that maps to your 400x500 coordinates
    ...
    <div className="glass-card rounded-lg p-4 space-y-3 border-t-2 border-primary/30">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          {inPractice && (
            <span className="text-xs font-semibold text-accent-foreground bg-accent rounded-full px-2 py-0.5 border border-accent-foreground/20 animate-pulse">
          ...
          {lastShot && isLocalPlayer && (
            <Button
              size="sm"
              variant="ghost"
              onClick={() => (mp.isMultiplayer ? mp.removeMultiplayerShot(lastShot.id) : removeShot(lastShot.id))}
              ...
              >
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    {teamDone && (
                      <span className="text-[10px] text-primary font-semibold">{t("shotTracker.done")}</span>
                    )}
                  </div>
                ...
                >
                  ...
                  <span className="flex items-center gap-1 w-full">
                    ...
                    <span className="flex-1 h-1 bg-secondary rounded-full overflow-hidden">
                      <span
                        className={`block h-full ${progressColor} rounded-full transition-all`}
          ...
          <AnimatePresence>
            {courtShots.map((shot) => (
              <motion.circle
                ...
                animate={{ scale: 1, opacity: shot.isPractice ? 0.5 : 0.85 }}
          ...
          {pendingPos && (
            ...
            >
              <Button
                ...
                className="bg-shot-made hover:bg-shot-made/80 text-primary-foreground font-bold shadow-lg shadow-green-500/30 ring-2 ring-green-400/20"
              >
                {t("shotTracker.made")}
              </Button>
              <Button
                onClick={() => confirmShot(false)}
                className="bg-shot-missed hover:bg-shot-missed/80 text-primary-foreground font-bold shadow-lg shadow-red-500/30 ring-2 ring-red-400/20"
              >
                {t("shotTracker.missed")}
              </Button>
              <Button variant="outline" onClick={() => setPendingPos(null)} className="text-xs">
      ...
      {activePlayer && pendingPos && (
        <p className="text-xs text-center text-muted-foreground">
          {inPractice && (
            <span className="text-accent-foreground font-semibold">{t("shotTracker.practicePrefix")} </span>
          )}
          {t("shotTracker.placingShot", {
            name: activePlayer.name,
            zone: zoneName(pendingPos.zone),
            points: ZONE_POINTS[pendingPos.zone],
          })}
        </p>
      )}
    </div>
